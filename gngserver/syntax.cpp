...
syntax_install (GNGServer *gngserver, lt_dlhandle module, Syntax *table)
{
  ...
  GNGS_ASSERT (table);
  std::cerr << "> syntax_install" << std::endl;
...
syntax_remove (GNGServer *gngserver, lt_dlhandle module, Syntax *table)
{
  ...
  std::cerr << "> syntax_remove" << std::endl;
...
syntax_handler (GNGServer *gngserver, int ch)
{
  ...
  std::cerr << "> syntax_handler(" << gngserver << ", " << ch << ")" << std::endl;

  sh = gngserver->syntax[ch] ? gngserver->syntax[ch]->handler : NULL;
  std::cerr << "< syntax_handler (returning " << *sh << ")" << std::endl;
