...
static GNGServerState *gngserverstate_find (GNGServer *gngserver, const char *key);
...
int
gngserverstate_set (GNGServer *gngserver, const char *key, const void *value,
...
gngserver_result_append (GNGServer *gngserver, const char *value, ...)
{
	...
	while (argument) {
		...
		if (gngserver->len + len >= gngserver->lim) {
			gngserver->lim += CHUNK_SIZE;
			gngserver->result =
				XREALLOC(char, gngserver->result, gngserver->lim);
		}
