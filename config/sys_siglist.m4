...
# serial 1 GNGS_VAR_SYS_SIGLIST

AC_DEFUN(GNGS_VAR_SYS_SIGLIST,
[AC_CACHE_CHECK([for sys_siglist],
gngs_cv_var_sys_siglist,
[AC_TRY_LINK([int *p;], [extern int sys_siglist; p = &sys_siglist;],
	    gngs_cv_var_sys_siglist=yes, gngs_cv_var_sys_siglist=no)])
if test x"$gngs_cv_var_sys_siglist" = xyes; then
